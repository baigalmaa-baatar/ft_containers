...
namespace ft
{
    ...
    private:
        ...
        */
        ...
        void rotateLeft(node_pointer *root, node_pointer x)
        {
        ...
        */
        ...
        node_pointer _insert(node_pointer node, node_pointer newNode)
        {
            ...
            if (node == ft_nullptr || node == this->_end)
            {
                return (newNode);
            }
            if (!this->_compare(node->key.first, newNode->key.first))
            ...
            else if (this->_compare(node->key.first, newNode->key.first))
            {
            ...
            _setHeight(node);
            // node->height = 1 + _max(_getHeight(node->left), _getHeight(node->right)); // do I need new funciton? in case of null subtrees?
