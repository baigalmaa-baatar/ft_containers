...
namespace ft
{
    ...
    private:
        ...
        node_pointer _insert(node_pointer existingNode, node_pointer newNode)
        {
            ...
            if (existingNode == ft_nullptr || existingNode == this->_end)
            {
                return (newNode);
            }
            if (!this->_compare(existingNode->key.first, newNode->key.first))
                // if (newNode->key < existingNode->key)
            {
                existingNode->left = _insert(existingNode->left, newNode);
                // if (existingNode->left)
                // {
                //     newNode->left = this->_end;
                //     this->_end->right = newNode;
                // }
                // existingNode->left = newNode;

                if (existingNode->left == newNode)
                    newNode->parent = existingNode;
            }
            else if (this->_compare(existingNode->key.first, newNode->key.first))
            {
                existingNode->right = _insert(existingNode->right, newNode);
                // if (existingNode->right)
                // {
                //     newNode->right = this->_end;
                //     this->_end->left = newNode;
                // }
                // existingNode->right = newNode;

                if (existingNode->right == newNode)
                    newNode->parent = existingNode;
            }
            else //the key already exists, thus we can't add new node.
            {
                return existingNode;
            }
            // newNode->parent = existingNode;
            _setHeight(existingNode);
            // existingNode->height = 1 + _max(_getHeight(existingNode->left), _getHeight(existingNode->right)); // do I need new funciton? in case of null subtrees?
            existingNode = _balanceTree(existingNode);
            return (existingNode);
        };
        ...
        node_pointer _remove(node_pointer root, T key)
        {
            ...
            else
            {
                ...
                if (root->left == ft_nullptr && root->right == ft_nullptr)
                {
                    ...
                    {
                        std::cout << "ELSE" << '\n';
                        this->_end = ft_nullptr; //need to check later
                        root = this->_end;
                        // this->_node_alloc.destroy(root);
                        // this->_node_alloc.deallocate(root, 1);
                        this->_root = this->_end;
    ...
    public:
        ...
        node_pointer insert_pos(node_pointer pos, T key) //need to check later
        {
